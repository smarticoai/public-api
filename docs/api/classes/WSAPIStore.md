...
Callback invoked with the full refreshed
                           catalog after every [buyStoreItem](#buystoreitem)
...
Promise resolving to the catalog of store
...
- `type = 'changelevel'` → user's level adjusted; surfaces via
  [getCurrentLevel](WSAPIUser.md#getcurrentlevel).
...
The store item `id` (from `getStoreItems()`).
...
- The same set of categories is returned to every user of the label — there is no
  ...
  segment visibility happens on [getStoreItems](#getstoreitems), not here.
...
> **getStorePurchasedItems**(`params?`): `Promise`\<[`TStoreItem`](../interfaces/TStoreItem.md)[]\>
...
Optional pagination + callback bag.

###### limit?

`number`

Page size. Defaults to `20`. Use `20` as the
...
Number of rows to skip from the newest end.
...
Promise resolving to an array of `TStoreItem` history entries
...
- No prerequisite calls — calling without first fetching
  [getStoreItems](#getstoreitems) or [getStoreCategories](#getstorecategories) is fine.
