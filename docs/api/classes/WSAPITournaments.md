...
Callback invoked with the full refreshed
                           ...
                           [registerInTournament](#registerintournament) round-trip on
...
Promise resolving to the array of
...
see [getClanTournamentPlayers](#getclantournamentplayers) for fetching the players list
...
[getTournamentInstanceInfo](#gettournamentinstanceinfo). Tournaments with
...
**UI guidance**: see [UI Guide — `getTournamentsList`](../_media/UIGuide_getTournamentsList.md).
...
The `instance_id` from a `TTournament`
                             returned by [getTournamentsList](#gettournamentslist).

#### Returns

`Promise`\<[`TTournamentDetailed`](../interfaces/TTournamentDetailed.md)\>

Promise resolving to `TTournamentDetailed`.
...
on an item returned by [getTournamentsList](#gettournamentslist). The method works
...
**UI guidance**: see [UI Guide — `getTournamentInstanceInfo`](../_media/UIGuide_getTournamentInstanceInfo.md).

**Visitor mode**: not supported. Use [getTournamentsList](#gettournamentslist) for
...
The tournament `instance_id` from a
...
**Preconditions**
Pass a valid `tournamentInstanceId` (read from `TTournament.instance_id`
returned by [getTournamentsList](#gettournamentslist)) and a `clanId` (read from
`TTournamentDetailed.clan_leaderboard[].clan_id` returned by
[getTournamentInstanceInfo](#gettournamentinstanceinfo)). The tournament should be
...
- The default Smartico UI fetches this once when the user opens the
  ...
  [getTournamentInstanceInfo](#gettournamentinstanceinfo)) does refresh via its own
  polling cadence.
...
**UI guidance**: see [UI Guide — `getClanTournamentPlayers`](../_media/UIGuide_getClanTournamentPlayers.md).
...
The `instance_id` from `TTournament`
...
**Preconditions**
Read the candidate from [getTournamentsList](#gettournamentslist) and gate the call
...
**Error codes** (in `err_code`, typed as [TournamentRegistrationError](../enumerations/TournamentRegistrationError.md))
...
[getTournamentInstanceInfo](#gettournamentinstanceinfo) is NOT auto-refreshed — if the
...
- Any operator-configured on-join prizes are distributed (e.g.
  ...
  (e.g. [getBonuses](#getbonuses)).

**UI guidance**: see [UI Guide — `registerInTournament`](../_media/UIGuide_registerInTournament.md).
